...
function extraer_token(cuerpo_html) {
    const coincidencia = String(cuerpo_html).match(/name="_token"\s+value="([^"]+)"/);
...
function iniciar_sesion() {
    ...
    for (let intento = 1; intento <= intentos_maximos_login; intento++) {
        const formulario = http.get(`${url_base}/login`, { tags: { endpoint: 'login_get' } });
        ...
        const login = http.post(
            ...
            { tags: { endpoint: 'login_post' }, redirects: 0, responseCallback: respuestas_login_esperadas }
        );
    ...
    tasa_login_exitoso.add(false);
    check(null, { 'login logrado dentro de los reintentos permitidos': () => false });
    return false;
}
...
export default function () {
    ...
    group('flujo autenticado', function () {
        ...
        check(panel, {
            ...
            'panel devuelve JSON': (r) => String(r.headers['Content-Type']).includes('json'),
        });
        ...
        const respuesta_es_json = String(panel.headers['Content-Type']).includes('json');
